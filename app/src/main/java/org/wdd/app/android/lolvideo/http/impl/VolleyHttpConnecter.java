package org.wdd.app.android.lolvideo.http.impl;
...
import org.wdd.app.android.lolvideo.http.HttpConnecter;
...
import org.wdd.app.android.lolvideo.http.StatusCode;
...
import org.wdd.app.android.lolvideo.utils.LogUtils;
...
public class VolleyHttpConnecter implements HttpConnecter {
    ...
    @Override
    public HttpSession sendImageRequest(final ActivityFragmentAvaliable host, HttpRequestEntry entry, final HttpConnectCallback callback) {
        ImageRequest request = new ImageRequest(entry.getUrl(), new Response.Listener<Bitmap>() {
            @Override
            public void onResponse(Bitmap bitmap) {
                if (!host.isAvaliable()) return;
                HttpResponseEntry responseEntry = new HttpResponseEntry();
                responseEntry.setStatusCode(StatusCode.HTTP_OK);
                responseEntry.setData(bitmap);
        ...
        request.setShouldCache(entry.shouldCache());
        request.setRetryPolicy(new DefaultRetryPolicy(entry.getTimeOut(),
                DefaultRetryPolicy.DEFAULT_MAX_RETRIES, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT));
        requestQueue.add(request);
        HttpSession session = new VolleyHttpSession(request, entry);
