package org.wdd.app.android.lolvideo.views.jazzy;
...
//	for (int i = 0; i < getChildCount(); i++) {
//		View v = getChildAt(i);
//		//			setRotation(v, -getRotation(v));
//		//			setRotationX(v, -getRotationX(v));
//		//			setRotationY(v, -getRotationY(v));
//		//
//		//			setTranslationX(v, -getTranslationX(v));
//		//			setTranslationY(v, -getTranslationY(v));
//
//		setRotation(v, 0);
//		setRotationX(v, 0);
//		setRotationY(v, 0);
...
//}
	...
	private void logState(View v, String title) {
		Log.v(TAG, title + ": ROT (" + getRotation() + ", " +
				getRotationX() + ", " +
				getRotationY() + "), TRANS (" +
				getTranslationX() + ", " +
				getTranslationY() + "), SCALE (" +
				getScaleX() + ", " + 
				getScaleY() + "), ALPHA " +
				getAlpha());
	}
	...
	protected void animateTablet(View left, View right, float positionOffset) {		
		if (mState != State.IDLE) {
			...
			if (right != null) {
				...
				setPivotY(right.getMeasuredHeight()*0.5f);
				setTranslationX(mTrans);
				setRotationY(mRot);
	...
	private void animateCube(View left, View right, float positionOffset, boolean in) {
		if (mState != State.IDLE) {
			if (left != null) {
				...
				setPivotX(left.getMeasuredWidth());
				setPivotY(left.getMeasuredHeight()*0.5f);
				setRotationY(mRot);
			}
			if (right != null) {
				manageLayer(right, true);
	...
	private void animateAccordion(View left, View right, float positionOffset) {
		if (mState != State.IDLE) {
			if (left != null) {
				...
				setPivotX(left.getMeasuredWidth());
				setPivotY(0);
				setScaleX(1-positionOffset);
			}
			if (right != null) {
				manageLayer(right, true);
				setPivotX(0);
				setPivotY(0);
				setScaleX(positionOffset);
			}
		}
	}
	...
	private void animateZoom(View left, View right, float positionOffset, boolean in) {
		if (mState != State.IDLE) {
			if (left != null) {
				...
				setPivotX(left.getMeasuredWidth()*0.5f);
				setPivotY(left.getMeasuredHeight()*0.5f);
				setScaleX(mScale);
				setScaleY(mScale);
			}
	...
	private void animateRotate(View left, View right, float positionOffset, boolean up) {
		if (mState != State.IDLE) {
			if (left != null) {
				...
				setPivotX(left.getMeasuredWidth()*0.5f);
				setPivotY(up ? 0 : left.getMeasuredHeight());
				setTranslationY(mTrans);
				setRotation(mRot);
			}
			if (right != null) {
				manageLayer(right, true);
				...
				mTrans = (up ? -1 : 1) * (float) (getMeasuredHeight() - getMeasuredHeight()*Math.cos(mRot*Math.PI/180.0f));
				setPivotX(right.getMeasuredWidth()*0.5f);
				setPivotY(up ? 0 : right.getMeasuredHeight());
				setTranslationY(mTrans);
				setRotation(mRot);
			}
		}
