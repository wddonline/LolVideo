package org.wdd.app.android.lolvideo.ui.matches.data;
...
public class MatchesDataGetter {
    ...
    private String mUrl = ServerApis.BASE_URL + "/bisai";

    public MatchesDataGetter(Context context, DataCallback callback) {
    ...
    public void requestMatchVideos(boolean isAppend, ActivityFragmentAvaliable host) {
        doRequest(mUrl, isAppend, host);
    }

    public void requestMatchVideos(String url, final boolean isAppend, final ActivityFragmentAvaliable host) {
    ...
    private void doRequest(String url, final boolean isAppend, final ActivityFragmentAvaliable host) {
        ...
        requestEntry.setUrl(url + "/list-" + mPage + ".html");
        ...
        mHttpManager.sendHtmlRequest("GB2312", host, requestEntry, new HttpConnectCallback() {
            ...
            public void onRequestOk(HttpResponseEntry res) {
                ...
                if (nodes.size() > 0) {
                    nodes = nodes.first().getElementsByTag("a");
                    if (nodes.size() > 0) {
                        if (nodes.last().text().contains("下一页")) {
                            isLastPage = false;
                            mPage++;
                        }
                    }
